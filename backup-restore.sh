...
REMNALABS_ROOT_DIR=""
SCRIPT_REPO_URL="https://raw.githubusercontent.com/chipikakao/remnawave-backup-restore/main/backup-restore.sh"
        ...
        case $choice in
            1)
                ...
                case "$bot_choice" in
                    ...
                    5) BOT_BACKUP_SELECTED="Бот от 3252a8"; bot_folder="remnawave-minishop" ;;
                    0) continue ;;
...
restore_bot_backup() {
    ...
    while true; do
        ...
        case "$bot_choice" in
            ...
            5) selected_bot_name="Бот от 3252a8"; break ;;
                ...
                if [[ "$AUTO_UPDATE" == "true" ]]; then
                    ...
                    if curl -fsSL "$SCRIPT_REPO_URL" -o "$TEMP_SCRIPT_PATH" 2>/dev/null; then
                        if [[ -s "$TEMP_SCRIPT_PATH" ]] && head -n 1 "$TEMP_SCRIPT_PATH" | grep -q -e '^#!.*bash'; then
                            ...
                            local release_url="https://github.com/chipikakao/remnawave-backup-restore/releases/tag/${REMOTE_VERSION_LATEST}"
